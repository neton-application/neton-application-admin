...
export function getJob(id: number) {
  return requestClient.get<InfraJobApi.Job>(`/infra/job/get/${id}`);
}
...
export function deleteJob(id: number) {
  return requestClient.delete(`/infra/job/delete/${id}`);
}
...
export function runJob(id: number) {
  return requestClient.put(`/infra/job/trigger/${id}`);
}

/** 获得定时任务的下 n 次执行时间 */
export function getJobNextTimes(id: number) {
  return requestClient.get(`/infra/job/get_next_times/${id}`);
}
